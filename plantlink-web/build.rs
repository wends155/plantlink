...
fn main() {
    ...
    if should_build {
        ...
        let arg1 = if cfg!(target_os = "windows") { "/C" } else { "-c" };
        
        // npm install
        let install_cmd = format!("cd ../ui && npm install");
        ...
        // npm run build
        let build_cmd = format!("cd ../ui && npm run build");
        let status = Command::new(shell)
            .args(&[arg1, &build_cmd])
            .status()
        ...
        println!("cargo:warning=Skipping UI build (Debug mode & dist exists). Run 'npm run build' manually if needed.");
    }
}
