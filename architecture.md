...
- **Structured Concurrency**: Remediated. The `RuntimeEngine` uses `JoinSet` and `CancellationToken` to manage node lifecycles. This ensures that `NodeBehavior::stop()` is called for all nodes upon termination and that all actor tasks are joined before the engine exits. Unstructured `tokio::spawn` calls in `inject.rs` are managed via explicit `JoinHandle::abort()` in `on_stop`.
- **Rhai Execution Limits**: Remediated. The script engine now enforces a 5,000 operation limit per execution, 100-element array limit, and 100-element map limit to prevent resource exhaustion and infinite loops in user scripts.
