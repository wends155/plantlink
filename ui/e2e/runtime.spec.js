...
test.describe('Runtime Engine E2E', () => {
    ...
    test('should execute Rhai script and observe mutated output via WebSocket', async ({ request, page }) => {
        ...
        const flowPayload = {
            nodes: [
                ...
                { id: 'n2', type: 'rhai', data: { name: 'rhai', code: 'msg.payload = msg.payload + " RHAI_OK"; return msg;' } },
    ...
    test('should reject malformed flow with unregistered node type', async ({ request }) => {
        const badFlow = {
            nodes: [
                { id: 'bad1', type: 'nonexistent_node_type', data: {} }
            ],
            edges: []
        };
    ...
    test('should report tasks_aborted > 0 when stopping an active flow', async ({ request }) => {
        ...
        const flowPayload = {
            nodes: [
                { id: 'n1', type: 'inject', data: { name: 'inject', payload: 'timer-test', interval: 0.5 } },
