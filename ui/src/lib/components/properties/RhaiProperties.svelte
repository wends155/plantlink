...
<div class="flex flex-col h-[300px] border border-[var(--color-border)] rounded overflow-hidden">
  <label
    class="label px-2 py-1 bg-[var(--color-bg-secondary)] border-b border-[var(--color-border)]"
  >
    Rhai Script
  </label>
  <div class="flex-1 overflow-auto">
    <CodeMirror bind:value={data.code} lang={rust()} theme={editorTheme} />
  </div>
  <div
    class="px-2 py-1 bg-[var(--color-bg-secondary)] border-t border-[var(--color-border)] flex justify-between items-center text-xs"
  >
