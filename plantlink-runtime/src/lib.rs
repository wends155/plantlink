...
mod nodes;
use nodes::{NodeBehavior, NodeContext};
use tokio::sync::mpsc;
...
impl RuntimeEngine {
    ...
    pub async fn update_flow(&mut self, flow: FlowConfig) {
        ...
        for edge in &flow.edges {
            ...
            wiring.entry(edge.source.clone())
                .or_default()
                .entry(src_port)
                .or_default()
        ...
        let mut node_senders: HashMap<String, HashMap<usize, Vec<(mpsc::Sender<(usize, MessagePayload)>, usize)>>> = HashMap::new();
        let mut node_receivers: HashMap<String, Vec<(usize, mpsc::Receiver<(usize, MessagePayload)>)>> = HashMap::new();
        
        // Use a Shared Resource Registry for this flow execution
        let resources = std::sync::Arc::new(tokio::sync::RwLock::new(std::collections::HashMap::new()));
        ...
        for (source_id, ports) in wiring {
            for (src_port, targets) in ports {
                for (target_id, tgt_port) in targets {
                    ...
                    node_senders.entry(source_id.clone())
                        .or_default()
                        .entry(src_port)
                        .or_default()
                        .push((tx, tgt_port));
                    ...
                    node_receivers.entry(target_id.clone())
                        .or_default()
                        .push((tgt_port, rx));
                }
            }
        ...
        for config in flow.nodes {
            ...
            let ctx = NodeContext::new(node_id.clone(), outputs, resources.clone(), self.tx.clone());
            
            // Create specific node instance dynamically from registry
            let mut node: Box<dyn NodeBehavior> = match nodes::registry::create_node(&config.type_, &config) {
                Ok(n) => n,
                Err(e) => {
            ...
            let task = tokio::spawn(async move {
                ...
                if !inputs.is_empty() {
                    ...
                    for (port_idx, rx) in inputs {
                         let stream = tokio_stream::wrappers::ReceiverStream::new(rx).map(|(_, msg)| msg);
